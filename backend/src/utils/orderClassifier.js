...
function normalizeAdminOrder(doc, orderKind = "shopping") {
  ...
  if (isShopping) {
    ...
    const fabricInfo = firstItem?.fabricType ? ` (${firstItem.fabricType})` : "";
    itemsSummary = itemCount > 0 ? `${itemCount} item${itemCount > 1 ? "s" : ""} • ${firstItemName}${fabricInfo}` : "Boutique Order";
